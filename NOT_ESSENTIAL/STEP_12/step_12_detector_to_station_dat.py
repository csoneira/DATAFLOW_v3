...
'''
Step 12: assign station/conf time ranges and emit station .dat files.

Inputs: detector-format .dat from Step 11.
Outputs: SIMULATED_DATA/mi0XYYDDDHHMMSS.dat and output registry.
'''
...
def sample_payloads(
    ...
    reservoir: list[str] = []
    reservoir_times: list[float] | None = None
    expected_fields = 7 + (4 * 4 * 4)
    seen = 0
        ...
        with path.open("r", encoding="ascii") as handle:
            for line in handle:
                ...
                if len(parts) < expected_fields:
                    continue
                payload_parts = parts[7:]
                thick_time_s = 0.0
                has_thick = len(parts) == expected_fields + 1
                if has_thick and payload_parts:
                    try:
                        thick_time_s = float(payload_parts[-1])
                ...
                if len(reservoir) < target_rows:
                    ...
                    if has_thick:
                        if reservoir_times is None:
                            reservoir_times = []
                        reservoir_times.append(thick_time_s)
                    continue
                ...
                if pick < target_rows:
                    ...
                    if has_thick:
                        if reservoir_times is None:
                            reservoir_times = [0.0] * target_rows
                        reservoir_times[pick] = thick_time_s
    return reservoir, seen, reservoir_times
...
def main() -> None:
    ...
    if args.plot_only:
        print("Plot-only requested; Step 12 does not generate plots. Skipping.")
        return
    ...
    payloads, total_rows, thick_offsets = sample_payloads(input_paths, target_rows, rng)
