...
def main() -> None:
    ...
    if args.plot_only:
        if args.no_plots:
            ...
            return
        for out_file in sorted(output_dir.rglob(f"SIM_RUN_*/geom_*_daq.{output_format}")):
            df, _ = load_with_metadata(out_file)
            plot_path = out_file.with_name(f"{out_file.stem}_plots.pdf")
            plot_jitter_summary(df, plot_path)
            print(f"Saved {plot_path}")
        for manifest_path in sorted(output_dir.rglob("SIM_RUN_*/geom_*_daq.chunks.json")):
    ...
    if chunk_rows:
        ...
        if not args.no_plots and plot_df is not None:
            ...
            plot_jitter_summary(plot_df, plot_path)
