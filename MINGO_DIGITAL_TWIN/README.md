...
- STEP_1: Blank -> Generated
  ...
  - Purpose: generate primary muon tracks (position/direction/time) using standard flux/angle distributions
...
- STEP_2: Generated -> Crossing
  ...
  - Purpose: propagate muon trajectories, compute per-plane incidence time and position
...
- STEP_3: Crossing -> Avalanche
  ...
  - Purpose: simulate RPC gas ionization in the active volume (avalanche size/position/time), no readout effects
...
- STEP_4: Avalanche -> Hit
  ...
  - Purpose: use avalanche centers/sizes to induce strip signals and affected readout strips
...
- STEP_5: Hit -> Signal (T_diff/q_diff)
  ...
  - Purpose: compute charge imbalance and time-difference from strip X position
...
- STEP_6: Signal -> Front/Back
  ...
  - Purpose: propagate strip signals to both ends (front/back times and charges)
...
- STEP_7: Front/Back -> Calibrated
  ...
  - Purpose: apply cable/connector propagation offsets (uncalibration step)
...
- STEP_8: Threshold (FEE model)
  ...
  - Purpose: simulate FEE thresholding and charge-to-time conversion (uncalibration step)
...
- STEP_9: Trigger
  ...
  - Purpose: apply coincidence trigger logic and keep allowed plane combinations
...
- STEP_10: DAQ timing model
  ...
  - Purpose: apply TDC smear and DAQ clock jitter to coincidence events
...
- STEP_FINAL: DAQ -> Station Dat (final formatting)
  ...
  - Purpose: emulate the event builder and emit station-style .dat output
...
- input_sim_run accepts explicit SIM_RUN_<N>, latest, or random (runtime config).
- Steps skip recomputation if the matching SIM_RUN exists unless --force is provided.
- STEP_1 flux_cm2_min and STEP_3 efficiencies can be lists; one option is chosen per run and used for SIM_RUN identity.
- Columns are preserved across steps; STEP_FINAL is the only format-shaping step.
