...
Script: MINGO_DIGITAL_TWIN/VALIDATION/validators/validate_step5_strip_obs.py
Purpose: Validator for STEP 5 strip observables (T_diff and q_diff).
Owner: DATAFLOW_v3 contributors
Sign-off: csoneira <csoneira@ucm.es>
Last Updated: 2026-03-02
Runtime: python3
Usage: python3 MINGO_DIGITAL_TWIN/VALIDATION/validators/validate_step5_strip_obs.py [options]
...
def run(
    ...
    if qdiff_width_raw in (None, ""):
        rb.add(
            test_id="step5_qdiff_width_configured",
            test_name="STEP 5 qdiff_width configured",
            metric_name="qdiff_width_configured",
            metric_value=0,
            status="FAIL",
            notes="STEP 5 metadata is missing qdiff_width.",
        )
        return rb.to_frame().reindex(columns=RESULT_COLUMNS)
