...
def calibrate_strip_Q_pedestal(Q_ch, T_ch, Q_other, self_trigger_mode = False):
    ...
    for number_of_bins in bin_vector:
        # print(f"Number of bins: {int(number_of_bins)}")
    ...
    # Tranform offsets to a numpy array
    ...
    if use_max:
        ...
        offset = ( bin_edges[max_bin_index] + bin_edges[max_bin_index + 1] ) / 2
    else:
        offset = np.median(offsets)

    print(f"Chosen offset: {offset}")
    ...
    if create_plots:
        ...
        plt.axhline(y=offset, color='r', linestyle='--', label=f'Chosen Offset: {offset:.2f}')
        plt.axhline(y=np.median(offsets), color='g', linestyle='--', label=f'Median Offset: {np.median(offsets):.2f}')
        plt.legend()
        plt.title("Histogram of Offsets")
