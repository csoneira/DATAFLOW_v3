...
echo "[run_tasks_filtering] Starting $(basename "$STEP_DIR")"

mapfile -t task_dirs < <(find "$STEP_DIR" -maxdepth 1 -mindepth 1 -type d -name 'TASK_*' | sort)

if (( ${#task_dirs[@]} == 0 )); then
  echo "[run_tasks_filtering] No TASK_* directories found in $STEP_DIR"
  exit 0
fi
...
for task_dir in "${task_dirs[@]}"; do
  ...
  for py_script in "${py_scripts[@]}"; do
    echo "[run_tasks_filtering] Running ${py_script#$STEP_DIR/}"
    python3 "$py_script"
  done
done

echo "[run_tasks_filtering] Completed $(basename "$STEP_DIR")"
