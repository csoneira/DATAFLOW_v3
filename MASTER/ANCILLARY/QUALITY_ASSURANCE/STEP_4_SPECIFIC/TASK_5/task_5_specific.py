#!/usr/bin/env python3
"""TASK_5 specific QA: generate grouped plots and pass placeholders."""
...
DEFAULT_PASS_COLUMN = "task_5_specific_pass"
        ...
        if patterns:
            ...
            if "ncols" in item:
                try:
                    ...
                    pass
            if "overlay_before_after" in item:
                group["overlay_before_after"] = _to_bool(item.get("overlay_before_after"), False)
            if "before_suffix" in item:
                group["before_suffix"] = str(item.get("before_suffix", "")).strip()
            if "after_suffix" in item:
                group["after_suffix"] = str(item.get("after_suffix", "")).strip()
            groups.append(group)

    return groups or _default_plot_groups(config)


def _pair_sort_key(base_name: str) -> tuple[int, int, str]:
    metric_order = {
        "T_sum_nonzero": 0,
        "T_diff_nonzero": 1,
        "Q_sum_nonzero": 2,
    ...
    if base_name.startswith("P") and "_" in base_name:
        plane_token, metric_name = base_name.split("_", 1)
        plane_text = plane_token[1:]
        if plane_text.isdigit():
            plane_idx = int(plane_text)
    return (plane_idx, metric_order.get(metric_name, 99), metric_name)


def _resolve_before_after_pairs(
    columns: list[str],
    before_suffix: str,
    after_suffix: str,
    ...
    for column_name in columns:
        if before_suffix and column_name.endswith(before_suffix):
            base = column_name[: -len(before_suffix)]
            pairs.setdefault(base, {})["before"] = column_name
        elif after_suffix and column_name.endswith(after_suffix):
            base = column_name[: -len(after_suffix)]
            pairs.setdefault(base, {})["after"] = column_name

    out: list[tuple[str, str | None, str | None]] = []
    for base in sorted(pairs.keys(), key=_pair_sort_key):
        out.append((base, pairs[base].get("before"), pairs[base].get("after")))
    return out
...
def _plot_group(
    *,
    ...
    overlay_before_after: bool = False,
    before_suffix: str = "_before_filter6_rate_hz",
    after_suffix: str = "_after_filter6_rate_hz",
) -> Path | None:
    ...
    plot_cfg = config.get("plots") if isinstance(config.get("plots"), dict) else {}
    if group_ncols is None:
        configured_ncols = int(plot_cfg.get("ncols", 2))
    else:
        configured_ncols = int(group_ncols)
    ...
    if overlay_before_after:
        pairs = _resolve_before_after_pairs(plot_columns, before_suffix, after_suffix)
        if not pairs:
            return None
        ncols = max(1, min(configured_ncols, len(pairs)))
        ...
        for idx, (base_name, before_col, after_col) in enumerate(pairs):
            ax = axes_flat[idx]
            plotted_any = False

            if before_col is not None:
                y_before = pd.to_numeric(df[before_col], errors="coerce")
                if y_before.notna().any():
                    ax.plot(
                        x_values,
                        y_before,
                        linestyle="-",
                    ...
                    ax.plot([], [], color="#1f77b4", label="before (no data)")
            else:
                ax.plot([], [], color="#1f77b4", label="before (missing)")

            if after_col is not None:
                y_after = pd.to_numeric(df[after_col], errors="coerce")
                if y_after.notna().any():
                    ax.plot(
                        x_values,
                        y_after,
                        linestyle="-",
                    ...
                    ax.plot([], [], color="#ff7f0e", label="after (no data)")
            else:
                ax.plot([], [], color="#ff7f0e", label="after (missing)")
    ...
    fig.suptitle(f"TASK_{TASK_ID} {station_name} specific - {group_name} (x={x_label})", fontsize=11)
    if x_is_datetime:
...
def _generate_station_plots(station_name: str, config: dict[str, Any], meta_df: pd.DataFrame) -> list[Path]:
    ...
    for group in groups:
        ...
        out_path = _plot_group(
            ...
            overlay_before_after=bool(group.get("overlay_before_after", False)),
            before_suffix=str(group.get("before_suffix", "_before_filter6_rate_hz")),
            after_suffix=str(group.get("after_suffix", "_after_filter6_rate_hz")),
        )
