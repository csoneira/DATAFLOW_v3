#!/usr/bin/env python3
"""TASK_1 specific QA: generate requested specific plots and pass placeholders."""
...
DEFAULT_PASS_COLUMN = "task_1_specific_pass"
...
def _plot_zeroed_percentage_grid(
    *,
    df: pd.DataFrame,
    x_values: pd.Series,
    x_label: str,
    ...
    zeroed_cfg = plot_cfg.get("zeroed_grid", {}) if isinstance(plot_cfg.get("zeroed_grid"), dict) else {}
    nrows = int(zeroed_cfg.get("nrows", 4))
    ncols = int(zeroed_cfg.get("ncols", 4))
    figsize = tuple(zeroed_cfg.get("figsize", [18, 12]))
    ...
    for plane in range(1, 5):
        for strip in range(1, 5):
            ...
            ax = axes_flat[idx]
            col = f"zeroed_percentage_P{plane}s{strip}"
            if col in df.columns:
                has_data = _plot_series_or_empty(
                    ax=ax,
                    x_values=x_values,
                    y_values=df[col],
                    label="zeroed_pct",
                ...
                ax.text(0.5, 0.5, "missing", transform=ax.transAxes, ha="center", va="center", color="red", fontsize=8)
    ...
    fig.suptitle(f"TASK_{TASK_ID} {station_name} specific - zeroed percentages (x={x_label})", fontsize=11)
    if x_is_datetime:
        fig.autofmt_xdate(rotation=30)
    fig.tight_layout(rect=(0, 0, 1, 0.96))

    plots_dir.mkdir(parents=True, exist_ok=True)
    out_path = plots_dir / f"{station_name}_task_{TASK_ID}_{METADATA_TYPE}_zeroed_percentage_grid.{image_format}"
...
def _plot_entries_grid(
    *,
    ...
    nrows = int(entries_cfg.get("nrows", 8))
    ncols = int(entries_cfg.get("ncols", 4))
    figsize = tuple(entries_cfg.get("figsize", [18, 24]))
    image_format = str(plot_cfg.get("format", "png")).strip().lower()
    dpi = int(plot_cfg.get("dpi", 150))
    marker_size = float(plot_cfg.get("marker_size", 2.0))
    line_width = float(plot_cfg.get("line_width", 0.9))
    ...
    for plane in range(1, 5):
        for side in ("F", "B"):
            row = (plane - 1) * 2 + (0 if side == "F" else 1)
            for strip in range(1, 5):
                col_idx = strip - 1
                idx = row * ncols + col_idx
                ax = axes_flat[idx]

                col_original = _first_existing_column(
                    df,
                    [
                        f"{prefix}{plane}_{side}_{strip}_entries_original_rate_hz",
                        f"{prefix}{plane}_{side}_{strip}_entries_original",
                    ],
                )
                col_final = _first_existing_column(
                    df,
                    [
                        f"{prefix}{plane}_{side}_{strip}_entries_final_rate_hz",
                        f"{prefix}{plane}_{side}_{strip}_entries_final",
                    ],
                )
                panel_title = f"{prefix}{plane}_{side}_{strip} (orig+final)"

                has_any_series = False
                missing_labels: list[str] = []

                if col_original is not None:
                    original_label = "original" if col_original.endswith("_rate_hz") else "original (count)"
                    has_data = _plot_series_or_empty(
                        ax=ax,
                        x_values=x_values,
                    ...
                    has_any_series = has_any_series or has_data
                else:
                    missing_labels.append("original")

                if col_final is not None:
                    final_label = "final" if col_final.endswith("_rate_hz") else "final (count)"
                    has_data = _plot_series_or_empty(
                        ax=ax,
                        x_values=x_values,
                    ...
                    missing_labels.append("final")

                if not has_any_series and not missing_labels:
                    ax.text(0.5, 0.5, "no data", transform=ax.transAxes, ha="center", va="center", color="gray", fontsize=8)
                if missing_labels:
                    ax.text(
                        0.5,
    ...
    for idx in range(nrows * ncols, len(axes_flat)):
        axes_flat[idx].axis("off")

    fig.suptitle(
        f"TASK_{TASK_ID} {station_name} specific - {prefix} entries rates (x={x_label})",
        fontsize=11,
    )
    if x_is_datetime:
        fig.autofmt_xdate(rotation=30)
    ...
    plots_dir.mkdir(parents=True, exist_ok=True)
    out_path = plots_dir / f"{station_name}_task_{TASK_ID}_{METADATA_TYPE}_{prefix}_entries_grid.{image_format}"
    fig.savefig(out_path, dpi=dpi)
    plt.close(fig)
    return out_path


def _tq_rates_identical(df: pd.DataFrame) -> bool:
    pairs_compared = 0
    for plane in range(1, 5):
        for side in ("F", "B"):
            for strip in range(1, 5):
                for state in ("original", "final"):
                    t_col = _first_existing_column(
                        df,
                        [
                            f"T{plane}_{side}_{strip}_entries_{state}_rate_hz",
                            f"T{plane}_{side}_{strip}_entries_{state}",
                        ],
                    )
                    q_col = _first_existing_column(
                        df,
                        [
                            f"Q{plane}_{side}_{strip}_entries_{state}_rate_hz",
                            f"Q{plane}_{side}_{strip}_entries_{state}",
                        ],
                    )
                    ...
                    q_num = pd.to_numeric(df[q_col], errors="coerce")
                    pairs_compared += 1

                    same = ((t_num.isna() & q_num.isna()) | (t_num == q_num)).all()
                    if not same:
                        return False
...
def _generate_station_plots(station_name: str, config: dict[str, Any], meta_df: pd.DataFrame) -> list[Path]:
    ...
    skip_q_if_identical = bool(
        (plot_cfg.get("entries_grid", {}) if isinstance(plot_cfg.get("entries_grid"), dict) else {}).get(
            "skip_q_if_identical_to_t", True
        )
    )

    if skip_q_if_identical and _tq_rates_identical(df):
        print(f"TASK_{TASK_ID} {station_name}: skipping Q entries figure (identical to T).")
    else:
        created.append(
            _plot_entries_grid(
                prefix="Q",
