#!/usr/bin/env python3
"""TASK_3 specific QA: generate grouped plots and pass placeholders."""
...
DEFAULT_PASS_COLUMN = "task_3_specific_pass"
...
def _resolve_overlay_panels(
    ...
    for pattern in overlay_patterns:
        ...
        if star is None:
            series_maps.append({pat: pat} if pat in df.columns else {})
            continue
    ...
    for key in panel_keys:
        panels.append(
            {
                "title": key,
                "columns": [mapping.get(key) for mapping in series_maps],
            }
        )
    return panels


def _default_plot_groups() -> list[dict[str, Any]]:
    return [
        {"name": "raw_rates", "columns": ["raw_tt_*_rate_hz"]},
        {"name": "clean_rates", "columns": ["clean_tt_*_rate_hz"]},
        {"name": "raw_to_clean_rates", "columns": ["raw_to_clean_tt_*_rate_hz"]},
        {"name": "raw_counts", "columns": ["raw_tt_*_count"]},
        {"name": "clean_counts", "columns": ["clean_tt_*_count"]},
        {"name": "raw_to_clean_counts", "columns": ["raw_to_clean_tt_*_count"]},
        {"name": "other", "columns": ["count_rate_denominator_seconds"]},
    ]
    ...
    for idx, item in enumerate(groups_cfg, start=1):
        ...
        if isinstance(overlay_columns, list):
            ...
            if overlay_patterns:
                ...
                panel_rows_raw = item.get("panel_rows")
                panel_rows: list[list[str]] = []
                if isinstance(panel_rows_raw, list):
        ...
        if isinstance(columns, list):
            ...
            if patterns:
                groups.append({"name": name, "mode": "columns", "columns": patterns})

    return groups or _default_plot_groups()
...
def _plot_group(
    *,
    ...
    nrows = int(math.ceil(len(plot_columns) / max(ncols, 1)))
    fig_width = float(plot_cfg.get("figsize_per_col", 4.2)) * max(ncols, 1)
    fig_height = float(plot_cfg.get("figsize_per_row", 2.8)) * max(nrows, 1)

    fig, axes = plt.subplots(nrows=nrows, ncols=max(ncols, 1), figsize=(fig_width, fig_height), sharex=True)
...
def _plot_overlay_group(
    *,
    ...
    out_dir: Path,
    config: dict[str, Any],
    panel_rows: list[list[str]] | None = None,
) -> Path | None:
    if not panels:
        return None
    ...
    line_width = float(plot_cfg.get("line_width", 0.9))
    image_format = str(plot_cfg.get("format", "png")).strip().lower()
    panel_lookup = {str(panel.get("title", "")): panel for panel in panels}
    series_count = len(series_labels)
    if series_count <= 0 and panels:
        series_count = len(panels[0].get("columns", []))
    fallback_columns = [None] * max(series_count, 1)

    def _draw_overlay_panel(ax: plt.Axes, panel: dict[str, Any], panel_idx: int) -> None:
        title = str(panel.get("title", f"panel_{panel_idx + 1}"))
        columns = panel.get("columns", [])
        if not isinstance(columns, list):
            columns = []
        if len(columns) < len(fallback_columns):
            columns = list(columns) + [None] * (len(fallback_columns) - len(columns))
        ...
        for series_idx, col in enumerate(columns):
            ...
            if y.notna().any():
                ...
                # Keep legend entry even when the requested column has no values.
    ...
    use_panel_rows = bool(panel_rows)
    if use_panel_rows:
        nrows = len(panel_rows or [])
        ncols = max((len(row) for row in (panel_rows or [])), default=0)
        if nrows <= 0 or ncols <= 0:
            return None
        fig_width = float(plot_cfg.get("figsize_per_col", 4.2)) * ncols
        fig_height = float(plot_cfg.get("figsize_per_row", 2.8)) * nrows
        fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(fig_width, fig_height), sharex=True)
        axes_2d = np.atleast_2d(axes)
        for row_idx, row_keys in enumerate(panel_rows or []):
            for col_idx in range(ncols):
                ax = axes_2d[row_idx, col_idx]
                if col_idx >= len(row_keys):
                    ax.axis("off")
                    continue
                key = str(row_keys[col_idx]).strip()
                panel = panel_lookup.get(key, {"title": key, "columns": fallback_columns})
                _draw_overlay_panel(ax, panel, row_idx * ncols + col_idx)
    else:
        configured_ncols = int(plot_cfg.get("ncols", 4))
        ncols = max(1, min(configured_ncols, len(panels)))
        ...
        for idx, panel in enumerate(panels):
            ax = axes_flat[idx]
            _draw_overlay_panel(ax, panel, idx)
...
def _generate_station_plots(station_name: str, config: dict[str, Any], meta_df: pd.DataFrame) -> list[Path]:
    ...
    for group in groups:
        ...
        if group.get("mode") == "overlay":
            overlay_patterns = group.get("overlay_columns", [])
            panels = _resolve_overlay_panels(df, overlay_patterns)
            out_path = _plot_overlay_group(
                ...
                out_dir=plots_dir,
                config=config,
                panel_rows=group.get("panel_rows"),
            )
        else:
