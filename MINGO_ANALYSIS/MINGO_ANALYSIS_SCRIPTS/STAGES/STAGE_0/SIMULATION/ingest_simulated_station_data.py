...
Script: MINGO_ANALYSIS/MINGO_ANALYSIS_SCRIPTS/STAGES/STAGE_0/SIMULATION/ingest_simulated_station_data.py
Purpose: !/usr/bin/env python3.
Owner: DATAFLOW_v3 contributors
Sign-off: csoneira <csoneira@ucm.es>
Last Updated: 2026-03-02
Runtime: python3
Usage: python3 MINGO_ANALYSIS/MINGO_ANALYSIS_SCRIPTS/STAGES/STAGE_0/SIMULATION/ingest_simulated_station_data.py [options]
...
def main() -> None:
    ...
    master_dir = next(
        (p for p in current_path.parents if p.name == "MINGO_ANALYSIS_SCRIPTS"),
        None,
    )
    if master_dir is None:
        raise RuntimeError(f"Unable to resolve MASTER directory from {current_path}")
    repo_root = master_dir.parent.parent
    source_dir = Path(args.source_dir).expanduser().resolve()
    source_dir.mkdir(parents=True, exist_ok=True)
    if not source_dir.is_dir():
        raise NotADirectoryError(f"Source path is not a directory: {source_dir}")

    station_root = repo_root / "MINGO_ANALYSIS" / "MINGO_ANALYSIS_STATIONS" / "MINGO00"
